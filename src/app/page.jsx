...
import { Mail, ArrowUpRight, Instagram, Twitter, Linkedin } from 'lucide-react';
import { BlurReveal, ParallaxText } from '../components/ui/Animators';
    ...
    >
      <div className="relative w-screen h-screen">
        {images.map((src, i) => (
          ...
          >
            <img src={src.src || src} alt="Polaroid" className="w-full h-full object-cover grayscale" />
          </motion.div>
        ))}
      </div>
    </motion.div>
  ...
  return (
    <AnimatePresence mode="wait">
      {isVisible && (
        ...
        >
          <div className="bg-black/90 backdrop-blur-xl px-4 py-2.5 md:px-6 md:py-3 rounded-full border border-white/10 flex items-center justify-around md:justify-start gap-4 md:gap-6 shadow-2xl shadow-black/20 text-xs md:text-sm font-medium text-white/90 w-full">
...
const Hero = () => {
  ...
  const yCloud = useTransform(scrollYProgress, [0, 1], [0, 600]);
  const yText = useTransform(scrollYProgress, [0, 1], [0, 400]);

  return (
    <section id="home" ref={ref} className="relative z-20 min-h-dvh md:min-h-screen flex flex-col items-center justify-start md:justify-between pt-16 md:pt-32 overflow-visible">
      {/* Layer 0: Background Image (Sky) */}
      <div className="absolute inset-0 z-0">
        <img src={bgImg.src || bgImg} alt="Sky Background" className="w-full h-full object-cover" />
      </div>
      ...
      >
        <BlurReveal delay={3} disabled={isMobile}>
          <ParallaxText baseVelocity={10} className="w-full">
            <h1 className="text-[15vw] md:text-[12vw] flex items-center justify-center font-garamond-serif font-bold tracking-wide text-white leading-none">
              KARTIKEY
            </h1>
          </ParallaxText>
        </BlurReveal>
      ...
      <div className="relative z-10 w-full max-w-7xl mt-4 md:mt-25 h-[48vh] md:h-[65vh] flex flex-col items-center justify-end">

        {/* Layer 1: Clouds (Behind Hero) */}
        <motion.div
          style={{ y: yCloud }}
          className="absolute top-[10%] md:top-[15%] left-1/2 -translate-x-1/2 w-[90%] md:w-[70%] z-10 opacity-100"
        >
          <img src={cloudImg.src || cloudImg} alt="Clouds" className="w-full h-auto object-contain" />
        </motion.div>
        ...
        >
          ...
          {/* Fog/Blend Effects - Desktop only to avoid covering face on mobile */}
          <div className="hidden md:block absolute bottom-[-1px] left-1/2 -translate-x-1/2 w-[120vw] h-[30vh] bg-linear-to-t from-white via-white/90 to-transparent z-30" />
          <div className="hidden md:block absolute bottom-[-50px] left-1/2 -translate-x-1/2 w-[120vw] h-[15vh] backdrop-blur-xl z-30 mask-image-gradient-up" />
        </motion.div>
      </div>

      {/* FIXED BOTTOM FOG */}
      <div className="absolute bottom-0 left-0 w-full z-25 pointer-events-none">
        {/* Mobile Overlays */}
        ...
        <div className="hidden md:block">
          {/* Lowered solid white bottom block to fully show shoulders/chest */}
          <div className="absolute bottom-0 left-0 w-full h-[28vh] bg-white" />
          {/* Adjusted fade strip to smoothly blend suit jacket into white */}
          <div className="absolute bottom-[28vh] left-0 w-full h-[22vh] bg-linear-to-t from-white to-transparent" />
        </div>
      </div>

      {/* Quote Block Container */}
      <div className="relative z-30 mt-4 md:-mt-16 flex flex-col items-center pb-8">
        <BlurReveal delay={4}>
          <img
            src={quoteImg.src || quoteImg}
            alt="Quote"
...
const Philosophy = () => {
  ...
  const buttonOpacity = useTransform(scrollYProgress, [0.34, 0.42], [0, 1]);
  const buttonY = useTransform(scrollYProgress, [0.34, 0.42], [20, 0]);

  return (
    <section id="about" ref={containerRef} className="relative z-0 h-[220vh] md:h-[300vh] bg-white">
      <div className="sticky top-0 h-screen flex flex-col items-center justify-center overflow-hidden">
        <div className="max-w-4xl px-6 text-center z-10 flex flex-col items-center mt-0">
          <h2 className="text-4xl md:text-7xl font-bold tracking-tighter mb-6 md:mb-8 leading-[0.9]">
            <ScrollWordReveal progress={scrollYProgress} range={[0.02, 0.10]}>
              Philosophy
            </ScrollWordReveal>
          </h2>
          <div className="text-sm md:text-xl text-gray-500 leading-relaxed font-light max-w-5xl mx-auto space-y-4 md:space-y-8">
            <p>
              <ScrollWordReveal progress={scrollYProgress} range={[0.08, 0.16]}>
                I believe in building systems that compound skills, code, capital, and audience.
              </ScrollWordReveal>
            </p>
            <p className="text-xs md:text-lg">
              <ScrollWordReveal progress={scrollYProgress} range={[0.12, 0.24]}>
            ...
            <p className="text-xs md:text-lg font-medium text-black">
              <ScrollWordReveal progress={scrollYProgress} range={[0.20, 0.32]}>
            ...
            <motion.div style={{ opacity: buttonOpacity, y: buttonY }}>
              <button className="bg-black text-white px-5 py-1.5 md:px-6 md:py-2 rounded-full text-xs md:text-base font-medium hover:bg-zinc-800 transition-colors mt-2 md:mt-4">
                Let’s build something that ships.
              </button>
            </motion.div>
            <div className="text-[10px] md:text-xs text-gray-400 mt-2">©Kartikey srivastava</div>
          </div>
        </div>
        ...
        <motion.div style={{ y: y1, rotate: -12 }} className="hidden md:block absolute left-[10%] top-[40%] w-64 h-80 z-20">
          <img src={scatter2.src || scatter2} alt="" className="w-full h-full object-cover rounded-sm shadow-2xl border-4 border-white" />
        </motion.div>
        <motion.div style={{ y: y2, rotate: 6 }} className="hidden md:block absolute right-[15%] top-[60%] w-56 h-72 z-20">
          <img src={scatter3.src || scatter3} alt="" className="w-full h-full object-cover rounded-sm shadow-2xl border-4 border-white" />
        </motion.div>
        <motion.div style={{ y: y3, rotate: -6 }} className="absolute left-[20%] top-[80%] w-48 h-64 z-0 opacity-50 blur-sm">
          <img src={scatter1.src || scatter1} alt="" className="w-full h-full object-cover rounded-sm" />
        </motion.div>
      </div>
    </section>
  );
...
const BentoGrid = () => (
  <section id="work" className="relative z-30 py-32 px-6 max-w-7xl mx-auto bg-white mt-0 md:mt-[-50vh]">
    ...
    <div id="projects" className="grid grid-cols-1 md:grid-cols-3 gap-6 auto-rows-auto md:auto-rows-[400px]">
      ...
      >
        <img src={scatter2.src || scatter2} className="absolute inset-0 w-full h-full object-cover opacity-60 group-hover:scale-105 transition-transform duration-700" alt="Agriculture AI" />
      ...
      >
        ...
        <img src={bento1.src || bento1} className="absolute inset-0 w-full h-full object-cover opacity-30 mix-blend-overlay group-hover:rotate-3 transition-transform duration-700" alt="AI Agent" />
      ...
      >
        <div className="absolute inset-0">
          ...
          <img src={scatter4.src || scatter4} className="absolute right-0 top-0 w-2/3 h-full object-cover opacity-20 grayscale group-hover:grayscale-0 transition-all duration-700" alt="Web Dev" />
        </div>
...
const Footer = () => (
  <footer className="py-12 px-6 border-t border-gray-100 max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-8 bg-white relative z-30">
    ...
    <div className="flex gap-6">
      <a href="#" className="p-3 bg-gray-55 rounded-full hover:bg-gray-100 transition-colors"><Instagram size={20} /></a>
      <a href="#" className="p-3 bg-gray-55 rounded-full hover:bg-gray-100 transition-colors"><Twitter size={20} /></a>
      <a href="#" className="p-3 bg-gray-55 rounded-full hover:bg-gray-100 transition-colors"><Linkedin size={20} /></a>
    </div>
